	...
	if ($a == 'info') {
		if (!empty($_POST['name']) && !empty($_POST['email']) && !empty($_POST['gender'])) {
			if (isset($_POST['name']) && isset($_POST['email']) && isset($_POST['gender'])) {
				...
				$check_update_email = $Tumdconnect->query("SELECT id FROM ".ACCOUNTS." WHERE id != '{$userData['id']}' AND email = '{$update_email}'");
				$update_gender = '';
						...
						if ($update_name != NULL && $update_email != NULL) {
							if (preg_match("/^[a-zA-Z ]+$/", $update_name)) {
								if (strlen($update_name) <= 20) {
									...
									$Tumdconnect->query("UPDATE ".ACCOUNTS." SET name='{$update_name}', email='{$update_email}', last_update_info='{$time}' WHERE id='{$userData['id']}'") or die();
									$Tumdconnect->query("UPDATE ".USERS." SET gender='{$update_gender}', about='{$update_about}' WHERE user_id='{$userData['id']}'") or die();
									$data['status'] = 200;
        ...
        if (isset($_POST['theme_pilot'])) {
            if (!empty($_POST['theme_pilot'])) {
                ...
                if ($sql_theme_query->num_rows > 0) {
                    ...
                    $Tumdconnect->query("UPDATE ".ACCOUNTS." SET profile_theme='{$theme_data['theme_class']}' WHERE id='{$userData['id']}'");
                    $data['status'] = 200;
            ...
            if (isset($_POST['current_password']) && isset($_POST['new_password']) && isset($_POST['new_password_v'])) {
                ...
                if ($v_pass == $userData['password']) {
                    if ($_POST['new_password'] == $_POST['new_password_v']) {
                        ...
                        $Tumdconnect->query("UPDATE ".ACCOUNTS." SET password='{$password_encryption}' WHERE id='{$userData['id']}'");
                        $data['status'] = 200;
