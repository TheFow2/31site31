								...
								if (filter_var($user_email, FILTER_VALIDATE_EMAIL)) {
									if ($name != NULL && $user_name != NULL && $user_password != NULL) {
										if (preg_match("/^[a-zA-Z- ]+$/", $name)) {
											if (strlen($name) <= 20) {
												$Tumdconnect->query("INSERT INTO ".ACCOUNTS." (name, username, password, email, ip, registration_date, xp) VALUES ('{$name}', '{$user_name}', '{$password_encryption}', '{$user_email}', '{$user_ip}', '{$time}', '{$config['xp_register']}')") or die();
												$ins_user_id = $Tumdconnect->insert_id;
												$Tumdconnect->query("INSERT INTO ".USERS." (user_id, gender) VALUES ({$ins_user_id},'1')");

												setcookie("tumd_ac_u", $ins_user_id, time() + (60 * 60 * 24 * 1), '/');
