<?php
	...
	if (!empty($_GET['id'])) {
		...
		if ( $get_profile_data == true ) {
			...
			$get_user_games_played = $Tumdconnect->query("SELECT DISTINCT game_id FROM ".USER_GAME." WHERE user_id='{$get_profile_data['id']}' AND type='played' AND game_id!='0' ORDER BY date_added DESC LIMIT 10");
			if ($get_user_games_played->num_rows > 0) {
				$gmplyd_r = '';
				while ( $games_played_row = $get_user_games_played->fetch_array() ) {
					$get_games_played = $Tumdconnect->query("SELECT * FROM ".GAMES." WHERE game_id='{$games_played_row['game_id']}' AND published='1'");
					while( $games_played = $get_games_played->fetch_array() ){
						...
						$gmplyd_r .= \Tumder\UI::view('profile/game-played-list');
					}
				}
				$themeData['profile_game_played_list'] = $gmplyd_r;
			} else {
				$themeData['profile_game_played_list'] = \Tumder\UI::view('profile/game-played-notfound');
			}
			# >>

			$get_user_favorite_games = $Tumdconnect->query("SELECT game_id FROM ".USER_GAME." WHERE user_id='{$get_profile_data['id']}' AND type='favorite' AND game_id!='0'");
			if ($get_user_favorite_games->num_rows > 0) {
				$gmfvt_r = '';
				while ( $games_favorite_row = $get_user_favorite_games->fetch_array() ) {
					$get_game_favorite = $Tumdconnect->query("SELECT * FROM ".GAMES." WHERE game_id='{$games_favorite_row['game_id']}' AND published='1'");
					while( $games_favorite = $get_game_favorite->fetch_array() ){
						...
						$gmfvt_r .= \Tumder\UI::view('profile/game-favorite-list');
					}
				}
				$themeData['profile_game_favorite_list'] = $gmfvt_r;
			} else {
				$themeData['profile_game_favorite_list'] = \Tumder\UI::view('profile/game-favorite-notfound');
			}

			$themeData['ads_sidebar'] = getADS('column_one');

			$themeData['page_content'] = \Tumder\UI::view('profile/content');

		} else { $themeData['page_content'] = \Tumder\UI::view('profile/error'); }
    } else { $themeData['page_content'] = \Tumder\UI::view('profile/error'); }