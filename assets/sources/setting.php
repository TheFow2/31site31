<?php
	...
	if ( is_logged() ) {
		...
		$themeData['setting_navigation_menu'] = \Tumder\UI::view('user/nav-menu');
		# >>
		...
		if (!isset($_GET['section']) or $_GET['section'] == "info") {
			...
			$themeData['setting_page_content'] = \Tumder\UI::view('user/info');
		} 
		elseif (isset($_GET['section']) && $_GET['section'] == "avatar") {
			...
			$themeData['setting_page_content'] = \Tumder\UI::view('user/avatar');
		} 
		elseif (isset($_GET['section']) && $_GET['section'] == "theme") {
			$sql_query_theme = $Tumdconnect->query("SELECT * FROM ".THEMES) or die();
			$i = 1;
			$thm_r = '';
			while ( $sql_theme_view = $sql_query_theme->fetch_array() ) {
				...
				$thm_r .= \Tumder\UI::view('user/theme-list');
			}
			$themeData['theme_list'] = $thm_r;
			$themeData['setting_page_content'] = \Tumder\UI::view('user/theme');
		}
		elseif (isset($_GET['section']) && $_GET['section'] == "password") {
			$themeData['setting_page_content'] = \Tumder\UI::view('user/password');
		} 
		else {
			$themeData['setting_page_content'] = \Tumder\UI::view('welcome/error-section');
		}

		$themeData['page_content'] = \Tumder\UI::view('user/setting');
	} else {
